...
class OpenCalendarKit_Admin_Settings {
    ...
    public static function get_settings() {
        ...
        if ( ! is_array( $settings ) ) {
            $settings = get_option( OpenCalendarKit_Plugin::LEGACY_OPTION_SETTINGS, [] );
        }
    ...
    public static function normalize_week_starts_on( $value ) {
        return in_array( $value, [ 'monday', 'sunday' ], true ) ? $value : self::defaults()['week_starts_on'];
    }

    public static function normalize_time_format_mode( $value ) {
        return in_array( $value, [ 'site_default', '24h', '12h' ], true ) ? $value : self::defaults()['time_format_mode'];
    }

    public static function resolve_bool( $value, $default ) {
        if ( $value === '' || $value === null ) {
            return (bool) $default;
        }
    ...
    public static function get_time_format( $mode = '' ) {
        $mode = self::normalize_time_format_mode( $mode ?: self::get( 'time_format_mode' ) );
