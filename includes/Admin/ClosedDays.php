...
class OpenCalendarKit_Admin_ClosedDays {
    ...
    public static function register_cpt() {
        OpenCalendarKit_I18n::with_locale(
            function () {
                ...
                add_filter( 'manage_' . OpenCalendarKit_Plugin::CPT_CLOSED_DAY . '_posts_columns', [ __CLASS__, 'cols' ] );
                add_action( 'manage_' . OpenCalendarKit_Plugin::CPT_CLOSED_DAY . '_posts_custom_column', [ __CLASS__, 'col_content' ], 10, 2 );
                add_filter( 'manage_edit-' . OpenCalendarKit_Plugin::CPT_CLOSED_DAY . '_sortable_columns', [ __CLASS__, 'sortable' ] );
                add_action( 'pre_get_posts', [ __CLASS__, 'default_order' ] );

                add_action( 'wp_ajax_' . OpenCalendarKit_Plugin::AJAX_SAVE_CLOSED_DAY, [ __CLASS__, 'ajax_save' ] );
                add_action( 'wp_ajax_' . OpenCalendarKit_Plugin::AJAX_DELETE_CLOSED_DAY, [ __CLASS__, 'ajax_delete' ] );
                add_action( 'wp_ajax_' . OpenCalendarKit_Plugin::AJAX_SAVE_OPEN_EXCEPTION, [ __CLASS__, 'ajax_save_open_exception' ] );
                add_action( 'wp_ajax_' . OpenCalendarKit_Plugin::AJAX_DELETE_OPEN_EXCEPTION, [ __CLASS__, 'ajax_delete_open_exception' ] );
            }
        );
    }
    ...
    private static function get_exception_post_ids( $date ): array {
        ...
        $ids = ! empty( $query->posts ) ? array_map( 'intval', $query->posts ) : [];
        wp_reset_postdata();
    ...
    private static function render_calendar_markup( $month = '' ) {
        ...
        <div class="bkit-calendar bkit-admin-cal" data-openkit-admin-calendar="1" data-month="<?php echo esc_attr( $date->format( 'Y-m' ) ); ?>" style="max-width:380px;">
            ...
            <div class="bkit-grid">
                ...
                $weekday_mon_first = array_merge( array_slice( $weekday_abbreviations, 1 ), [ $weekday_abbreviations[0] ] );
