    ...
    public static function render( $atts = [] ) {
        return OpenCalendarKit_I18n::with_locale(
            function () use ( $atts ) {
                ...
                $first_day_offset = $week_starts_on === 'sunday' ? ( $first_day_n % 7 ) : ( $first_day_n - 1 );
                    ...
                    <table class="bkit-cal-table" data-bk-cal>
                        ...
                        for ( $week = 0; $week < $weeks; $week++ ) :
                            ...
                            for ( $column = 1; $column <= 7; $column++ ) :
                                ...
                                $is_clickable = ! $cell['past'] && $cell['state'] === 'closed';
                                $reason = '';
                                if ( $cell['state'] === 'closed' ) {
                                    $reason = OpenCalendarKit_Admin_ClosedDays::get_reason( $cell['date'] );
                                }

                                $classes = 'bkit-cell day ' . ( $cell['past'] ? 'past disabled' : $cell['state'] ) . ( $is_clickable ? ' clickable' : '' );
